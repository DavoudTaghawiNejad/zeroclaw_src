...
async fn process_chat_message(
    ...
    if trimmed == "/new" || trimmed == "/clear" {
        ...
        let _ = sender
            .send(Message::Text(
                serde_json::json!({ "type": "chunk", "content": msg }).to_string().into(),
            ))
            .await;
        let _ = sender
            .send(Message::Text(
                serde_json::json!({ "type": "chunk_reset" }).to_string().into(),
            ))
            .await;
        let _ = sender
            .send(Message::Text(
                serde_json::json!({ "type": "done", "full_response": msg }).to_string().into(),
            ))
            .await;
        return;
    }
